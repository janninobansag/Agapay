...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  const filters = reportFiltersSchema.parse({ q: rawFilters.q ?? "", status: rawFilters.status || undefined });
  const { data: reports, availability } = await getResidentReports({ query: filters.q || undefined, status: filters.status });
  ...
  return (
    <div>
      ...
      <form className="mt-8 flex flex-col gap-3 rounded-2xl border border-border bg-surface p-3 sm:flex-row">
        <label className="flex flex-1 items-center gap-2 rounded-xl bg-surface-muted px-3"><Search className="text-muted" size={18} /><span className="sr-only">Search reports</span><input className="h-11 w-full bg-transparent text-sm outline-none placeholder:text-muted" defaultValue={filters.q} name="q" placeholder="Search by title, ID, category, or location" type="search" /></label>
        <label><span className="sr-only">Filter by status</span><select className="h-11 w-full rounded-xl border border-border bg-background px-3 text-sm sm:w-44" defaultValue={filters.status ?? ""} name="status"><option value="">All statuses</option><option value="DRAFT">Draft</option><option value="SUBMITTED">Submitted</option><option value="VERIFIED">Verified</option><option value="IN_PROGRESS">In progress</option><option value="RESOLVED">Resolved</option><option value="REJECTED">Rejected</option><option value="CANCELLED">Cancelled</option></select></label>
        <button className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-brand px-4 text-sm font-bold text-white hover:bg-brand-dark" type="submit"><Filter size={17} /> Apply</button>
        {(filters.q || filters.status) && <Link className="inline-flex h-11 items-center justify-center rounded-xl border border-border px-4 text-sm font-bold text-brand-dark" href="/reports">Clear</Link>}
      </form>
      <div className="mt-5 grid gap-4">
        ...
        {reports.length === 0 && availability === "ready" && (
          <div className="rounded-2xl border border-dashed border-border bg-surface p-12 text-center">
            <p className="font-bold text-brand-dark">{filters.q || filters.status ? "No reports match these filters" : "You have not created a report yet"}</p>
            <p className="mt-2 text-sm text-muted">{filters.q || filters.status ? "Try another search or clear the filters." : "Your drafts and submitted reports will appear here."}</p>
          </div>
        )}
      </div>
    </div>
